...
const {globSource,urlSource,CID} = require("ipfs-http-client")
...
class Ipfs {
  ...
  async add(data){
    let bdata
    if (data instanceof this.globSource){
      bdata = data
    }else if (data instanceof this.urlSource){
      bdata = data
    ...
    var cids=[]
    for await (const result of  this.client.add(bdata)){
       cids.push(result.cid.string)
    }
    return cids
  }
  async cat(cid){
  ...
  async blockGet(cid){
      var result = await this.client.block.get(cid)
      return result._data
  }
  async blockRm(cid){
    for await (const result of this.client.block.rm(cid)){
      console.log("rm:",result)
    }
  }
  async blockStat(cid){
    return this.client.block.stat(cid)
  }
